...
- Phần SGD hiện đã có cả `surface 3D`, `contour map`, và `combined view`.
- Mặt loss của SGD dùng pipeline riêng trong `geometry/sgd_visualizer.py`, không dùng lại `math_surface3d.py`.
- Shader của SGD cũng tách riêng ở `shaders/sgd_surface.vert/.frag` để dùng normal thật của bề mặt loss và cho cảm giác mềm, giống biểu đồ scientific hơn.
...
- voi hinh cau, cong thuc ly thuyet thuong la:
  - `u = theta / (2pi)`
  - `v = phi / pi`
- cach map nay phu hop nhat voi anh texture equirectangular ti le `2:1`
- neu nguoi dung dua anh thuong vao truc tiep, anh se bi keo ngang va de thay thanh "banh mat"
- vi vay code da bo sung buoc dem anh thuong len canvas `2:1` truoc khi upload texture cho sphere de giu ti le tu nhien hon
- neu thầy hoi tai sao van co the meo o gan cuc:
  - do ban chat phep chieu tu mat cau xuong mat phang 2D luon co bien dang, giong bai toan ban do dia cau
