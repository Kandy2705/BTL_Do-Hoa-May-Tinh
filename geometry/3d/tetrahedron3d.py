...
class Tetrahedron(BaseShape):
    def __init__(self, vert_shader, frag_shader):
        ...
        self.vertices, self.normals, self.colors, self.indices = self._generate_tetrahedron_geometry()
    ...
    def _generate_tetrahedron_geometry(self):
        vertices = np.array([
            [ 1,  1,  1],
            [-1, -1,  1],
            [-1,  1, -1],
        ...
        normals = vertices.copy() / np.linalg.norm(vertices, axis=1, keepdims=True)
        colors = np.array([[1.0, 1.0, 1.0]] * 4, dtype=np.float32)
        indices = np.array([
            0, 1, 2,
            0, 2, 3,
            0, 3, 1,
