...
class TruncatedCone(BaseShape):
    def __init__(self, vert_shader, frag_shader, radius_bottom=0.8, radius_top=0.4, height=1.0, slices=30, stacks=1):
        ...
        self.vertices, self.normals, self.colors = self._generate_truncated_cone()
    ...
    def _generate_truncated_cone(self):
        ...
        for i in range(self.stacks):
            ...
            for j in range(self.slices):
                ...
                # Tính normal cho mặt bên
                normal = np.array([np.cos(theta), (r_low - r_high) / self.height, np.sin(theta)])
                normal = normal / np.linalg.norm(normal)

                all_verts.extend([p1, p2, p3, p2, p4, p3])
                all_normals.extend([normal, normal, normal, normal, normal, normal])
                
                for _ in range(6):
                    all_colors.append([1.0, 1.0, 1.0])

        self._add_cap(all_verts, all_normals, all_colors, self.radius_bottom, -self.height / 2, clockwise=True)
        self._add_cap(all_verts, all_normals, all_colors, self.radius_top, self.height / 2, clockwise=False)
    ...
    def _add_cap(self, verts, normals, colors, radius, height, clockwise=True):
        center = [0.0, height, 0.0]
        center_normal = [0.0, 1.0 if height > 0 else -1.0, 0.0]
        ...
        for i in range(self.slices):
            ...
            normals.extend([center_normal, center_normal, center_normal])
